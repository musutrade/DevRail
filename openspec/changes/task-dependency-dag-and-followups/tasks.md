...
- [x] 7.4 使用明确文件清单提交并推送；推送前检查当前分支 PR 是否已合并，已合并则创建新 PR，未合并则更新现有 PR，并记录提交 SHA、分支和 PR URL。
- [x] 7.5 监控 PR 的 CI、arc-flow、供应链检查和 CodeQL；失败时读取日志、修复、重新执行相关测试和 `cargo flow verify --all`、提交并推送，循环直到所有必需检查成功。
