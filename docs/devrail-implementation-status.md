...
| 工具命令审批 | 部分实现 | 已有审批表、数据范围 API、审批中心列表/详情、批准/拒绝决策、过期时间、追加决策审计和 Supervisor resolve；撤回/过期 worker、通知和策略版本强校验仍待补齐。 |
...
2. Phase 1：补齐审批撤回、过期 worker/通知、中断后的 thread/resume 可恢复状态、changeset 和质量门禁，并完善运行验收；
