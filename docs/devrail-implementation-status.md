...
| Codex `app-server` Harness Supervisor | 基础实现 | 后端独占启动受控 `codex app-server`，完成初始化等待、thread/turn 启动、thread/resume、传输断流自动恢复（最多 2 次）、活动 run 数据库重启恢复、超时、stderr 摘要、恢复建议和优雅中断；审批等待状态仍需通知与人工恢复验收。 |
...
2. Phase 1：补齐过期通知和更丰富的质量门禁日志后端，并完善数据库重启恢复与运行验收；审批撤回、过期 worker、策略版本校验、changeset/质量门禁查询、受限命令质量门禁执行、结构化门禁元数据、质量门禁失败联动、SSE 心跳/断线补拉、活动 run 自动恢复和传输断流自动恢复已完成。
