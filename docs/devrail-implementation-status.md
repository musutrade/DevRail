...
- Symphony 第一阶段任务调度：后端 worker 按 queued 任务优先级、截止时间和创建顺序轮询，使用 PostgreSQL `SKIP LOCKED` + 可过期 claim 租约避免多实例重复领取，只调度启用环境并复用 Harness Supervisor 的受控启动和并发上限；容量暂满时任务保留在队列中自动重试；
...
| Symphony 任务调度器 | 第一阶段已实现 | DevRail 内部 `queued` 任务已支持优先级排序、启用环境过滤、活动 run 排除、数据库 claim 租约和过期恢复，并由后端 Supervisor 独占启动；尚未接入外部 issue tracker adapter、仓库级 `WORKFLOW.md` 动态加载和完整 Symphony tracker reconciliation。 |
| Codex `app-server` Harness Supervisor | 基础实现 | 后端独占启动受控 `codex app-server`，完成初始化等待、thread/turn 启动、thread/resume、传输断流自动恢复（最多 2 次）、活动 run 数据库重启恢复、审批等待状态人工恢复、超时、stderr 摘要、恢复建议和优雅中断。 |
...
| MVP 自动化验收 | 未完成 | 全量工程门禁通过不代表 requirements.md 第 16 节全部条件通过；Webhook 标准载荷和 GitHub 原生载荷归一化单元测试已补齐，端到端供应商回调演练仍待完成。 |
...
2. Phase 1：继续补齐过期通知和更丰富的质量门禁日志后端，并完善数据库重启恢复与运行验收；DevRail 内部 queued 任务调度第一阶段已完成；外部 issue tracker adapter、仓库级 `WORKFLOW.md` 动态加载、tracker 状态 reconciliation 和完整 Symphony 兼容性仍待实现。审批撤回、审批等待人工恢复、过期 worker、策略版本校验、changeset/质量门禁查询、受限命令质量门禁执行、结构化门禁元数据、质量门禁失败联动、SSE 心跳/断线补拉、活动 run 自动恢复和传输断流自动恢复已完成。
