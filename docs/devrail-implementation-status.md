...
- continuation turns：已新增请求与 handoff 迁移、数据范围与权限、幂等 claim/取消、TaskTracker 状态投影、唯一 child run、同 thread 新 turn、重启对账、handoff 前置 cleanup、隔离 workspace 重建、终态通知/outbox、低基数指标和任务/运行详情 UI；Rust 真实 PostgreSQL/受控假 app-server/workspace 测试及 Angular Vitest、Playwright 专项测试已通过；
...
| DevRail Angular 功能页 | 部分实现 | 已有项目 CRUD、成员、策略、任务列表/详情、任务依赖按权限编辑（添加/删除/整批保存及环冲突反馈）、continuation 时间线与追加上下文、仓库/环境列表与详情、远端分支/提交同步、受控工作树状态检查、资源创建、审批列表/详情、运行详情谱系、通知中心和通知设置页面及生成 API 服务。 |
...
| MVP 自动化验收 | 未完成 | continuation 代码与专项测试已落地，最近一次 backend 门禁为 111 个 Rust 测试通过，前端专项测试和 Playwright 桌面/移动视口通过；全量 `cargo flow verify --all`、端到端供应商回调/生产运行演练、受控修复 run 和 requirements.md 第 16 节完整验收仍待完成。 |
...
2. Phase 1：DevRail DB tracker 的 Symphony P0 调度可靠性、TaskTracker 抽象、仓库级 `WORKFLOW.md` 严格加载/动态 reload、不可变 task/run 快照、数据库重启恢复、断流/stall 运行验收、任务依赖 DAG、终态传播、受控 follow-up、per-task workspace/hooks 和 continuation turns 已完成；下一步实现受控修复 run。审批撤回、审批等待人工恢复、过期 worker、策略版本校验、changeset/质量门禁查询、受限命令质量门禁执行、结构化门禁元数据、质量门禁失败联动和 SSE 断线补拉已完成。
