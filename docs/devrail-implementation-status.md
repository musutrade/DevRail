...
- Phase 3 协作基础能力：任务评论持久化、数据范围查询、评论发布、编辑、软删除、变更审计、`@用户名` 提及解析、站内通知/outbox 和任务详情页评论区已加入；代码审查请求、指定审查人、通过/驳回决策、组织边界、审计、运行详情页审查区和逐文件意见已加入；补丁导出和 Git 平台集成仍待开发；
...
| 代码审查 | 基础实现 | 审查请求、逐文件意见（文件路径、意见、作者编辑）、权限、运行关联、列表、通过/驳回决策、事务审计和运行详情页入口已加入；补丁导出和 Git 平台同步仍待开发。 |
...
4. Phase 3：评论、提及、编辑、软删除、代码审查和逐文件意见基础闭环已加入；继续开发补丁导出和可选 Git 平台集成。
