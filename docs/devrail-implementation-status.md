...
- Phase 3 协作基础能力：任务评论持久化、数据范围查询、评论发布、编辑、软删除、变更审计、`@用户名` 提及解析、站内通知/outbox 和任务详情页评论区已加入；审查任务、补丁导出和 Git 平台集成仍待开发；
...
| 评论与提及 | 基础实现 | 任务评论 API、权限、数据范围、提及通知、编辑、软删除、审计和任务详情页评论区已加入；审查流程仍待开发。 |
...
4. Phase 3：评论、提及、编辑和软删除基础闭环已加入；继续开发审查、补丁导出和可选 Git 平台集成。
