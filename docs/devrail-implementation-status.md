...
| 任务、快照和运行 | 部分实现 | 已有任务详情页、服务端关键词/状态筛选、分页、不可变任务快照、run 生命周期字段、单任务活动运行唯一约束、幂等创建、终态重试、指定 turn 恢复和运行详情页；负责人/标签筛选、仓库环境关联和完整状态验收仍待补齐。 |
...
| DevRail Angular 功能页 | 部分实现 | 已有项目 CRUD、成员、策略、任务列表/详情、运行详情页面及生成 API 服务；仓库、环境、审批、通知页面仍待补齐。 |
...
1. Phase 0：完成项目/仓库/环境/任务 CRUD 的集成测试、负责人/标签筛选、成员与策略 API，并完成验收闭环；
2. Phase 1：补齐审批中心 UI、审批过期/通知、中断后的 thread/resume 可恢复状态、changeset 和质量门禁，并加入运行详情前端页；
