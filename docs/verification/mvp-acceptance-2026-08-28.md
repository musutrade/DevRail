...
- 执行时间：2026-09-02T08:11:22Z；命令：`cargo flow verify --all`；历史记录称各工程门禁通过，但对应本地 reports 文件未纳入版本控制，因此不能单独作为当前验收证据。后续证据按[验证证据格式](evidence-format.md)归档。
...
- 当时的本地 reports 文件未纳入版本控制；可复核范围以同段列出的提交与远端 CI 链接为准。
