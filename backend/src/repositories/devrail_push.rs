...
pub(crate) async fn claim_dispatches(
    ...
    sqlx::query("WITH candidates AS (SELECT o.id FROM devrail_outbox_events o WHERE (o.event_type='notification.created' OR o.payload ? 'notificationId') AND o.status IN ('pending','processing') AND o.available_at<=now() ORDER BY o.id FOR UPDATE SKIP LOCKED LIMIT $1) UPDATE devrail_outbox_events o SET status='processing', attempts=o.attempts+1 WHERE o.id IN (SELECT id FROM candidates)")
        .bind(limit).execute(&mut *tx).await?;
    sqlx::query("INSERT INTO devrail_push_deliveries (outbox_event_id,push_device_id,status) SELECT o.id,d.id,'pending' FROM devrail_outbox_events o JOIN devrail_notifications n ON n.organization_id=o.organization_id AND (n.id::text=o.payload->>'notificationId' OR n.source_key=o.payload->>'notificationSource') JOIN devrail_notification_preferences p ON p.organization_id=n.organization_id AND p.user_id=n.recipient_user_id AND p.push_enabled=true JOIN devrail_push_devices d ON d.organization_id=n.organization_id AND d.user_id=n.recipient_user_id AND d.status='active' WHERE o.status='processing' ON CONFLICT (outbox_event_id,push_device_id) DO NOTHING")
        .execute(&mut *tx).await?;
    let rows = sqlx::query_as::<_, DevRailPushDispatchRow>("SELECT d.id AS delivery_id,d.outbox_event_id,pd.id AS device_id,pd.user_id,pd.endpoint_ciphertext,pd.keys_ciphertext,n.title,n.summary,n.deep_link,n.id AS notification_id,COALESCE(o.payload->>'eventType',o.event_type) AS event_type FROM devrail_push_deliveries d JOIN devrail_push_devices pd ON pd.id=d.push_device_id JOIN devrail_outbox_events o ON o.id=d.outbox_event_id JOIN devrail_notifications n ON n.organization_id=o.organization_id AND (n.id::text=o.payload->>'notificationId' OR n.source_key=o.payload->>'notificationSource') WHERE d.status IN ('pending','retrying') AND o.status='processing' ORDER BY d.id FOR UPDATE SKIP LOCKED LIMIT $1")
